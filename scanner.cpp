...
Token* Scanner::nextToken() {
    ...
    if (isdigit(c)) {
        ...
        while (current < input.length() && isdigit(input[current]))
            current++;
        // Detectar float: dígitos seguidos de '.' y más dígitos
    ...
    // ID
        ...
        else if (lexema=="false" || lexema=="False") return new Token(Token::FALSE, input, first, current - first);
        else if (lexema=="true"  || lexema=="True")  return new Token(Token::TRUE,  input, first, current - first);
